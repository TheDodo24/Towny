...
public class TownyEconomyHandler {
	...
	public static Boolean setupEconomy() {
		 ...
		 */
		try {
			Plugin adventuria = Bukkit.getPluginManager().getPlugin("Adventuria");
			if(adventuria != null) {
				List<RegisteredServiceProvider<?>> vaultEcoProvider = plugin.getServer().getServicesManager().getRegistrations(adventuria);
				if(vaultEcoProvider.size() > 0) {
					if(vaultEcoProvider.stream().anyMatch(registeredServiceProvider -> registeredServiceProvider.getService().equals(Economy.class))) {
						RegisteredServiceProvider<Economy> economyProviderVault = (RegisteredServiceProvider<Economy>) vaultEcoProvider.stream().filter(provider -> provider.getService().equals(Economy.class)).findFirst().get();
						vaultEconomy = economyProviderVault.getProvider();
						setVersion(String.format("%s %s", economyProviderVault.getProvider().getName(), "via Vault (Adventuria check)"));
						Type = EcoType.VAULT;
						return true;
					}
				}
